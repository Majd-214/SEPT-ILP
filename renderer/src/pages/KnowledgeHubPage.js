 ...
 * and filterable by kind, each row linking to its own static topic page.
 */
export class KnowledgeHubPage extends Page {
  ...
  topbarItems() {
    return Html.el('span', { class: 'c-topbar__current' }, 'Knowledge base');
  }
  ...
  main() {
    ...
    return Html.el('div', { class: 'o-container' },
      ...
      this.repository.knowledgeDomains.map((domain) => this.#domain(domain)),
  ...
  #domain(domain) {
    return Html.el('section', { class: 'c-kb-domain', 'data-kb-domain': true, 'aria-label': domain.title },
